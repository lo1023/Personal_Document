#맥에서 win32com 라이브러리를 사용하지 못해 잠정 중단한다.

import os, sys, ctypes # 파이썬에서 라이브러리에 밑줄이 그어지면 설차가 안된것이다.
import win32com.client # ~COM은 Windows 전용이다. COM은 윈도우즈 레지스트리와 WinAPI를 광범위하게 사용해야 한다.
import pandas as pd
